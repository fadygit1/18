...
const EditOperationForm: React.FC<EditOperationFormProps> = ({ operation, clients, onSave, onCancel }) => {
  ...
  const [items, setItems] = useState<OperationItem[]>(operation.items);
  const [deductions, setDeductions] = useState<Deduction[]>(operation.deductions);
  const [guaranteeChecks, setGuaranteeChecks] = useState<GuaranteeCheck[]>(operation.guaranteeChecks);
  const [guaranteeLetters, setGuaranteeLetters] = useState<GuaranteeLetter[]>(operation.guaranteeLetters);
  const [receivedPayments, setReceivedPayments] = useState<ReceivedPayment[]>(operation.receivedPayments);
